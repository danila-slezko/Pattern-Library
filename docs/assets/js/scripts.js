...
(function () {
  ...
  function initAdaptNav(nav) {
    ...
    window.addEventListener("keyup", function (event) {
      if ((event.keyCode && event.keyCode == 9) || (event.key && event.key.toLowerCase() == "tab")) {
        //close dropdown if focus is outside dropdown element
  ...
  function getListWidth(list) {
    // get total width of container minus right padding
  ...
  function getFullWidth(item) {
    // get width of 'More Links' button
  ...
  function placeDropdown(nav) {
    // make sure dropdown is visible the viewport
  ...
  function modifyClone(clone) {
    // assign new classes to cloned elements inside the dropdown
...
(function () {
  ...
  if (crossTables.length > 0) {
    ...
    for (var i = 0; i < crossTables.length; i++) {
      ...
      if (beforeContent && beforeContent != "" && beforeContent != "none") {
        (function (i) {
          crossTablesArray.push(new CrossTables(crossTables[i]));
        })(i);
        j = j + 1;
      }
    }
    ...
    if (j > 0) {
      ...
      function doneResizing() {
        for (var i = 0; i < crossTablesArray.length; i++) {
          (function (i) {
            crossTablesArray[i].element.dispatchEvent(customEvent);
          })(i);
        }
      }
...
(function () {
  ...
  function initCalendarEvents(datePicker) {
    ...
    if (datePicker.trigger) {
      datePicker.trigger.addEventListener("click", function (event) {
    ...
    window.addEventListener("keydown", function (event) {
      ...
      if ((event.keyCode && event.keyCode == 27) || (event.key && event.key.toLowerCase() == "escape")) {
        if (document.activeElement.closest(".js-date-picker")) {
          ...
          // do not move focus -> only close calendar
  ...
  function getCurrentDay(date) {
    return date ? getDayFromDate(date) : new Date().getDate();
  }

  function getCurrentMonth(date) {
    return date ? getMonthFromDate(date) : new Date().getMonth();
  }

  function getCurrentYear(date) {
    return date ? getYearFromDate(date) : new Date().getFullYear();
  }
...
(function () {
  ...
  function resetTableStyle(table) {
    // update table style according to layout
    for (var i = 0; i < table.rows.length; i++) {
  ...
  if (tables.length > 0) {
    ...
    for (var i = 0; i < tables.length; i++) {
      ...
      if (beforeContent && beforeContent != "" && beforeContent != "none") {
        (function (i) {
          arrayTables.push(new ExTable(tables[i]));
        })(i);
        j = j + 1;
      } else {
    ...
    if (j > 0) {
      ...
      function doneResizing() {
        for (var i = 0; i < arrayTables.length; i++) {
          (function (i) {
            arrayTables[i].element.dispatchEvent(customEvent);
          })(i);
        }
      }
...
(function () {
  ...
  function initImageMagnifierScale(imgMag) {
    ...
    if (customScale) {
      // use custom scale set in HTML
      imgMag.scale = customScale;
    } else {
      // use natural width of image to get the scale value
...
(function () {
  ...
  function updateMediaHeight(element) {
    ...
    element.scrollDelta = window.innerHeight - element.visibleFigure.offsetHeight > window.innerWidth - element.visibleFigure.offsetWidth ? (window.innerHeight - element.visibleFigure.offsetHeight) / 2 : (window.innerWidth - element.visibleFigure.offsetWidth) / 2;
...
(function () {
  ...
  function initLoad(infiniteScroll) {
    ...
    if (infiniteScroll.options.container) {
      // get container of dynamic content
      infiniteScroll.container = infiniteScroll.element.querySelector(infiniteScroll.options.container);
    }

    if ((!infiniteScroll.options.loadBtn || infiniteScroll.options.loadBtnDelay) && infiniteScroll.loadBtn.length > 0) {
      // hide load more btn
    ...
    if (!infiniteScroll.options.path) {
      // content has been loaded using a custom function
  ...
  function setPathValues(infiniteScroll) {
    // path can be strin or comma-separated list
  ...
  function getTresholdPixel(infiniteScroll) {
    // get the threshold value in pixels - will be used only if intersection observer is not supported
  ...
  function initObserver(infiniteScroll) {
    ...
    var config = {
      rootMargin: "0px 0px " + infiniteScroll.options.threshold + " 0px",
    };
  ...
  function handleEvent(event) {
    ...
    switch (event.type) {
      ...
      case "scroll": {
        // triggered only if intersection onserver is not supported
        initScroll(this);
        break;
      }
    }
  ...
  function initScroll(infiniteScroll) {
    // listen to scroll event (only if intersectionObserver is not supported)
  ...
  function initBtnEvents(infiniteScroll) {
    // load more button events - click + focus (for keyboard accessibility)
  ...
  function initClick(infiniteScroll, event) {
  ...
  function checkLoad() {
    // while scrolling -> check if we need to load new content (only if intersectionObserver is not supported)
    if (this.loading) return;
    if (!needLoad(this)) return;
    loadMore(this);
  }

  function loadMore(infiniteScroll) {
    // new conten needs to be loaded
  ...
  function getFilePath(infiniteScroll) {
    // get path of the file to load
    return Array.isArray(infiniteScroll.options.path) ? infiniteScroll.options.path[infiniteScroll.index] : infiniteScroll.options.path.replace("{n}", infiniteScroll.index + 1);
  }
  ...
  function insertNewContent(infiniteScroll, content, moveFocus) {
    ...
    if (infiniteScroll.container) {
      ...
      infiniteScroll.loader.length > 0 ? infiniteScroll.loader[0].insertAdjacentHTML("beforebegin", content) : infiniteScroll.element.insertAdjacentHTML("beforeend", content);
    }
  ...
  function checkNextPageAvailable(infiniteScroll, checkNext) {
    // check if there's still content to be loaded
  ...
  function contentWasLoaded(infiniteScroll, url, checkNext) {
    // new content has been loaded - reset status
    if (infiniteScroll.loader.length > 0) Util.addClass(infiniteScroll.loader[0], "is-hidden"); // hide loader

    if (infiniteScroll.options.updateHistory && url) {
    ...
    if (!checkNext) {
      // no need to load additional pages - remove scroll listening and/or click listening
    ...
    if (checkNext && infiniteScroll.options.loadBtn) {
      // check if we need to switch from scrolling to click -> add/remove proper listener
    ...
    if (checkNext && infiniteScroll.loadBtn.length > 0 && Util.hasClass(infiniteScroll.loadBtn[0], "js-infinite-scroll__btn-focus")) {
      // keyboard accessibility
      Util.removeClass(infiniteScroll.loadBtn[0], "sr-only");
    }
  ...
  function needLoad(infiniteScroll) {
    // intersectionObserverSupported not supported -> check if threshold has been reached
    return infiniteScroll.element.getBoundingClientRect().bottom - window.innerHeight <= infiniteScroll.thresholdPixel;
  }

  function emitCustomEvents(infiniteScroll, eventName, moveFocus) {
    // applicable when user takes care of loading new content
    var event = new CustomEvent(eventName, {
      detail: { index: infiniteScroll.index + 1, moveFocus: moveFocus },
    });
  ...
  if (infiniteScroll.length > 0) {
    for (var i = 0; i < infiniteScroll.length; i++) {
      (function (i) {
        ...
        new InfiniteScroll({
          element: infiniteScroll[i],
          path: path,
          container: container,
          updateHistory: updateHistory,
          loadBtn: loadBtn,
          loadBtnDelay: loadBtnDelay,
          threshold: threshold,
        });
      })(i);
    }
...
(function () {
  ...
  function setAssetsSrc(asset) {
    for (var i = 0; i < asset.elements.length; i++) {
      if (asset.elements[i].getAttribute("data-bg") || asset.elements[i].tagName.toLowerCase() == "picture") {
        // this could be an element with a bg image or a <source> element inside a <picture>
...
(function () {
  ...
  function pauseLoopTabAutoplay(tab) {
    // pause autoplay
...
(function () {
  ...
  function initOffCanvas(panel) {
    ...
    for (var i = 0; i < panel.triggers.length; i++) {
      // lister to the click on off-canvas content triggers
  ...
  function cancelPanelEvents(panel) {
    //remove event listeners
  ...
  function initClick(panel, event) {
    // close panel when clicking on close button
  ...
  function getFocusableElements(panel) {
    //get all focusable elements inside the off-canvas content
  ...
  function getFirstVisible(panel, elements) {
    //get first visible focusable element inside the off-canvas content
    for (var i = 0; i < elements.length; i++) {
      if (elements[i].offsetWidth || elements[i].offsetHeight || elements[i].getClientRects().length) {
  ...
  function getLastVisible(panel, elements) {
    //get last visible focusable element inside the off-canvas content
    for (var i = elements.length - 1; i >= 0; i--) {
      if (elements[i].offsetWidth || elements[i].offsetHeight || elements[i].getClientRects().length) {
  ...
  function emitPanelEvents(panel, eventName, target) {
    // emit custom event
    var event = new CustomEvent(eventName, { detail: target });
...
(function () {
  ...
  if (percentageBar.length > 0) {
    for (var i = 0; i < percentageBar.length; i++) {
      (function (i) {
        new PercentageBar(percentageBar[i]);
      })(i);
    }
  }
})();
  ...
  function getRadialBarCode(chart) {
    for (var i = 0; i < chart.percentages.length; i++) {
      ...
      Util.setAttributes(circleBg, {
        ...
        cy: chart.width / 2,
        r: radius,
        class: "radial-bar__circle radial-bar__circle__bg",
        "data-index": i,
      });
...
(function () {
  ...
  if (rowTables.length > 0) {
    ...
    for (var i = 0; i < rowTables.length; i++) {
      ...
      if (beforeContent && beforeContent != "" && beforeContent != "none") {
        (function (i) {
          rowTablesArray.push(new RowTable(rowTables[i]));
        })(i);
        j = j + 1;
      }
    }
    ...
    if (j > 0) {
      ...
      function doneResizing() {
        for (var i = 0; i < rowTablesArray.length; i++) {
          (function (i) {
            rowTablesArray[i].element.dispatchEvent(customEvent);
          })(i);
        }
      }
    ...
    for (var i = 0; i < slider.rangeInput.length; i++) {
      (function (i) {
        ...
        slider.rangeInput[i].addEventListener("change", function (event) {
          // fix issue on IE where input event is not emitted
          updateSlider(slider, i);
        });
      })(i);
    }
...
(function () {
  function initSocialShare(button) {
    button.addEventListener("click", function (event) {
      ...
      social == "mail" ? (window.location.href = url) : window.open(url, social + "-share-dialog", "width=626,height=436");
    });
  }
  ...
  function getSocialUrl(button, social) {
    ...
    for (var i = 0; i < params.length; i++) {
      ...
      if (paramValue) {
        social == "facebook" ? (newUrl = newUrl + "u=" + encodeURIComponent(paramValue) + "&") : (newUrl = newUrl + params[i] + "=" + encodeURIComponent(paramValue) + "&");
      }
    }
...
(function () {
  ...
  function stickyCallback(entries, observer) {
    ...
    entries[0].target == this.scrollingElement ? (this.scrollingTreshold = threshold) : (this.nextTreshold = threshold);
...
(function () {
  ...
  function truncateAutocompleteList(element) {
    ...
    maxHeight > spaceBelow ? (element.resultsList.style.maxHeight = spaceBelow + "px") : (element.resultsList.style.maxHeight = "");
  }
  ...
  function setLabel(clone, label) {
    ...
    labelElement ? (labelElement.textContent = label) : (clone.textContent = label);
  }
...
(function () {
    ...
    for (var i = 1; i < yLabels.length; i++) {
      ...
      if (hideGuides) {
        Util.setAttributes(rectEl, {
          class: "chart__guides js-chart__guides opacity-0",
        });
      }
  ...
  function placexAxisLabels(chart) {
    ...
    if (chart.options.type == "column" && ticks[labels.length]) {
      Util.setAttributes(ticks[labels.length], {
        y1: chart.height - chart.bottomDelta - maxHeight - chart.svgPadding,
        y2: chart.height - chart.bottomDelta - maxHeight - chart.svgPadding + 5,
...
(function () {
  ...
  function resetCounter(voteCounter, pressed) {
    // update counter value (if present)
  ...
  function emitKeypressEvents(element, label, pressed) {
    // emit custom event when vote is updated
...
(function () {
  ...
  function startDateSelected(datePicker, day) {
    // new start date has been selected
...
(function () {
  ...
  function initDdf(element) {
    if (element.input.length > 0) {
      ...
      if (accept)
        element.options.acceptFile = accept.split(",").map(function (element) {
          return element.trim();
        });
    }
  ...
  function storeDroppedFiles(element, fileData) {
  ...
  function updateDndArea(element) {
  ...
  function filterMaxWeight(element) {
  ...
  function filterAcceptFile(element) {
  ...
  function formatInList(element, index) {
    ...
    for (var i = 0; i < element.options.acceptFile.length; i++) {
      ...
      if (extension && extensionInList(extension, element.options.acceptFile[i])) {
  ...
  function filterMaxFiles(element) {
  ...
  function updateDndAreaMessage(element) {
    ...
    if (element.droppedFiles.length > 0 && element.labelEndMessage) {
      ...
      if (element.labelEndMessage.length > 3) {
        finalMessage = element.droppedFiles.length > 1 ? element.labelEndMessage[0] + element.labelEndMessage[2] + element.labelEndMessage[3] : element.labelEndMessage[0] + element.labelEndMessage[1] + element.labelEndMessage[3];
      }
  ...
  function updateDndList(element) {
    ...
    if (element.options.replaceFiles) {
  ...
  function initRemoveFile(element) {
  ...
  function storeMultipleProgress(element) {
    // handle progress bar elements
    element.progress = [];
...
(function () {
  ...
  function initButtonTriggers(nav) {
    // mobile only
  ...
  function openFSideNav(nav, event) {
  ...
  function closeFSideNav(nav, event, bool) {
    // close side nav - mobile only
  ...
  function initAnchorEvents(nav) {
    nav.list.addEventListener("click", function (event) {
      ...
      if (nav.clickScrolling) {
        // a different link has already been clicked
        event.preventDefault();
        return;
      }
  ...
  if (fixedNav.length > 0) {
    for (var i = 0; i < fixedNav.length; i++) {
      (function (i) {
        fixedNavArray.push(new FSideNav(fixedNav[i]));
      })(i);
    }
    ...
    function doneScrolling() {
      for (var i = 0; i < fixedNavArray.length; i++) {
        (function (i) {
          fixedNavArray[i].element.dispatchEvent(customEvent);
        })(i);
      }
    }
  }
...
(function () {
  ...
  if (customSelect.length > 0) {
    for (var i = 0; i < customSelect.length; i++) {
      (function (i) {
        new CustomSelect(customSelect[i]);
      })(i);
    }
  }
})();
...
(function () {
  ...
  function updateCompareLabels(slider) {
    ...
    for (var i = 0; i < slider.captions.length; i++) {
      var delta = i == 0 ? slider.captions[i].offsetLeft - slider.modifiedImg.offsetLeft - slider.modifiedImg.offsetWidth : slider.modifiedImg.offsetLeft + slider.modifiedImg.offsetWidth - slider.captions[i].offsetLeft - slider.captions[i].offsetWidth;
...
(function () {
  ...
  function setMenuTabIndex(menu) {
    // set tabindexes for the menu items to allow keyboard navigation
    var nextItem = false;
...
(function () {
  ...
  function updateTriggerAria(select, ariaLabel) {
    // new label for custom triegger
...
(function () {
  ...
  function initOffCanvasNav(canvas) {
    ...
    if (canvas.svgAnim.length > 0) {
      // create the circle fill-in effect
...
(function () {
  ...
  function getStrengthLabels(password) {
    ...
    if (dataLabel) {
      ...
      password.strengthLabels = labels.map(function (element) {
        return element.trim();
      });
    }
  }
  ...
  function initInput(password) {
    password.input[0].addEventListener("input", function (event) {
      // password changed
      toggleCheckStrength(password); // hide/show password strenght section
      checkStrength(password);
      checkConditions(password);
  ...
  function checkStrength(password) {
    ...
    if (password.strengthValue.length > 0) {
    ...
    if (password.strengthMeter.length > 0) {
  ...
  if (passwordStrength.length > 0) {
    for (var i = 0; i < passwordStrength.length; i++) {
      (function (i) {
        new PasswordStrength({ element: passwordStrength[i] });
      })(i);
    }
  }
})();
  ...
  if (pTable.length > 0) {
    ...
    function updatePTableTeam(volumeOpt, value) {
      for (var i = 0; i < volumeOpt.length; i++) {
        volumeOpt[i].getAttribute("data-value") == value ? volumeOpt[i].removeAttribute("style") : volumeOpt[i].setAttribute("style", "display: none;");
      }
    }
  }
})();
...
(function () {
  ...
  function initProductV3(product) {
    ...
    if (product.cta.length > 0 && intObservSupported) {
      // detect when cta enters viewport
...
(function () {
  ...
  function updateRangeColor(slider) {
    // update background fill color of the slider
...
(function () {
  ...
  function getMaskPoints(slideshow, direction) {
    // store the path points - will be used to transition between slides
    var array = [];
  ...
  function initSlideshow(slideshow) {
    ...
    var srLiveArea = document.createElement("div");
    Util.setAttributes(srLiveArea, {
      class: "sr-only js-svg-slideshow__aria-live",
      "aria-live": "polite",
      "aria-atomic": "true",
    });
    slideshow.element.appendChild(srLiveArea);
  ...
  function showNewItem(slideshow, index, direction, bool) {
  ...
  function newItemAnimate(slideshow, newIndex, oldIndex, direction) {
    ...
    morphPath(path, mask, slideshow.options.transitionDuration, function () {
      // morph callback function
  ...
  function morphPath(path, points, duration, cb) {
    // morph
    if (reducedMotion || !animeJSsupported) {
      // if reducedMotion on or JS animation not supported -> do not animate
      for (var i = 0; i < path.length; i++) {
  ...
  function morphSinglePath(path, points, delay, duration, cb) {
    var dAnimation = points.length == 3 ? [{ value: [points[0], points[1]] }, { value: [points[1], points[2]] }] : [{ value: [points[0], points[1]] }];
    anime({
      targets: path,
      d: dAnimation,
      easing: "easeOutQuad",
  ...
  function updateAriaLive(slideshow, index) {
    // announce new selected slide to SR
...
(function () {
  ...
  function initLightboxMarkup(gallery) {
    ...
    for (var i = 0; i < gallery.galleryItems.length; i++) {
      ...
      var imgBlock = gallery.lazyload ? '<img data-src="' + src + '" data-loading="lazy" src="' + gallery.lazyload + '" ' + altAttr + " " + draggable + ">" : '<img src="' + src + '" data-loading="lazy" ' + draggable + ">";

      var captionBlock = caption.length > 0 ? '<figcaption class="exp-lightbox__caption">' + caption[0].textContent + "</figcaption>" : "";
  ...
  function hideSlideshowElements(gallery) {
  ...
  function initModalEvents(gallery) {
    ...
    if (gallery.slideshowObj.options.swipe) {
      // close gallery when you swipeUp/SwipeDown
  ...
  function runAnimation(gallery, imgInit, imgInitPosition, imgFinal, imgFinalPosition, bool) {
    ...
    Util.setAttributes(gallery.imgMorphImg, {
      "xlink:href": imgInit.getAttribute("src"),
      href: imgInit.getAttribute("src"),
    });
    Util.setAttributes(gallery.imgMorphRect, {
      style: "height: " + initHeight + "px; width: " + initWidth + "px;",
      transform: "translate(" + (initWidth / 2) * (1 - initScaleX) + " " + (initHeight / 2) * (1 - initScaleY) + ") scale(" + initScaleX + "," + initScaleY + ")",
    });
...
(function () {
  ...
  if (document.getElementById("explorer-link-variation")) {
    // --link variation
  ...
  if (explorer.length > 0) {
    ...
    new Autocomplete({
      ...
      onClick: function (option, obj, event) {
...
(function () {
  ...
  function initIntTable(table) {
    ...
    if (table.sortableCols.length > 0) {
      ...
      table.header.addEventListener("change", function (event) {
        // detect change in selected checkbox (SR only)
        var selectedCol = event.target.closest(".js-int-table__cell--sort");
  ...
  function toggleRowSelection(table) {
    // 'Select All Rows' checkbox has been selected/deselected
  ...
  function toggleAllSelection(table) {
    // Single row has been selected/deselected
    var allChecked = true,
      oneChecked = false;
    for (var i = 0; i < table.selectRow.length; i++) {
      if (!table.selectRow[i].checked) {
        allChecked = false;
      } else {
        oneChecked = true;
      }
  ...
  function setDataRowOrder(table) {
    // add a data-order to rows element - will be used when resetting the sorting
  ...
  function sortTableContent(table, order, index, selctedCol) {
    // determine the new order of the rows
    var rowsArray = table.body.getElementsByTagName("tr"),
      switching = true,
      i = 0,
      shouldSwitch;
...
(function () {
  ...
  function lazyLoadLightbox(modal) {
    ...
    modal.element.addEventListener("modalIsClose", function (event) {
      // add no-transition class
      Util.addClass(modal.element, "lightbox--no-transition");
    });
    // lazyload media of selected slide/prev slide/next slide
  ...
  function hideSlideshowElements(modal) {
    // hide slideshow controls if gallery is composed by one item only
  ...
  function resizeLightbox(modal) {
    // executed when window has been resized
  ...
  if (lightBoxes.length > 0) {
    ...
    for (var i = 0; i < lightBoxes.length; i++) {
      (function (i) {
        lightBoxArray.push(new Lightbox(lightBoxes[i]));
      })(i);
      ...
      function doneResizing() {
        for (var i = 0; i < lightBoxArray.length; i++) {
          (function (i) {
            resizeLightbox(lightBoxArray[i]);
          })(i);
        }
      }
      ...
      function updateLightbox(direction) {
        for (var i = 0; i < lightBoxArray.length; i++) {
          (function (i) {
            keyboardNavigateLightbox(lightBoxArray[i], direction);
          })(i);
        }
      }
    }
